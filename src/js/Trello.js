...
export default class Trello {
  ...
  todo() {
    ...
    this.container.addEventListener('mousedown', (e) => {
      ...
      if (e.target.classList.contains('task-item')) {
        ...
        this.temporaryTask = e.target.cloneNode(true);
        this.temporaryTask.classList.add('dragged');
        document.body.appendChild(this.temporaryTask);
        document.body.style.cursor = 'grabbing';
        this.temporaryTask.style.width = `${this.dragTask.offsetWidth}px`;
        const { top, left } = this.dragTask.getBoundingClientRect();
        this.calcTop = e.pageY - top;
        this.calcLeft = e.pageX - left;
        this.temporaryTask.style.top = `${top}px`;
        this.temporaryTask.style.left = `${left}px`;
      }
    });
    ...
    this.container.addEventListener('mousemove', (e) => {
      e.preventDefault();
      if (this.dragTask) {
        this.temporaryTask.style.left = `${e.pageX - this.calcLeft}px`;
        this.temporaryTask.style.top = `${e.pageY - this.calcTop}px`;
      }
    });
  ...
  stopMove() {
    document.body.removeChild(this.temporaryTask);
    this.temporaryTask = null;
    this.dragTask = null;
